...
impl TerminalBackend {
    pub fn new(
        ...
        // Use initial_size if provided, otherwise use default 80x50
        let terminal_size = if let Some(initial_size) = settings.initial_size {
            // Use a reasonable default font size for initial calculation
            // This will be updated when the terminal is first rendered
            let default_cell_width = 8.0f32;
            let default_cell_height = 14.0f32;
            
            let num_cols = (initial_size.width / default_cell_width).max(1.0) as u16;
            let num_lines = (initial_size.height / default_cell_height).max(1.0) as u16;
            
            TerminalSize {
                cell_width: default_cell_width as u16,
                cell_height: default_cell_height as u16,
                num_cols,
                num_lines,
                layout_size: initial_size,
            }
        } else {
